...
// var_dump($_SESSION);
// var_dump($_SESSION['chat_auth_code']);

if(empty($post_data['otp_sent_type'])){
	die(json_encode([
		MSG_RESULT => false,
		MSG_TYPE => MSG_ERROR,
		MSG_MSG => 'Please select one option.'
	]));
}
...
$response = do_cc_request('method=sendChatCode&'.$request_str);
