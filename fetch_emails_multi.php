...
$READ_EMAIL_BODY_IMAGE = "http://cc-robi.gplex.net/ccpro/read_email_body_image.php";
...
//                        var_dump($mail_content_i['subtype'] == 'PLAIN' && !empty($mail_content_i['data']));
    ...
    //}
    //var_dump($mail_body);
    $mail_body = changeImageURL($mail_body, $attachment_save_path, $now, $thread_id, $mail_id, $email_attachment_files);
    ...
    $session_id = generate_session_id($thread_id, $conn);

    generate_log_email_session($thread_id, $session_id, $skill_id, $from_email, $conn);
    $sql = "INSERT INTO email_messages SET ticket_id='$thread_id', mail_sl='$mail_id', original_mail_id='$original_mail_id', ".
        ...
        "reference='$reference_str', mail_body='$mail_body', has_attachment='$has_attachment', tstamp='$now', status='O', phone='$phone_number', session_id='$session_id' ";
...
function changeImageURL($mail_body, $attachment_save_path, $now, $thread_id, $mail_id, $email_attachment_files){
    ...
    // var_dump($altrs);
    // var_dump($matches);
    // var_dump($email_attachment_files);
    ...
    if(count($matches) > 0) {
        ...
        foreach($matches[1] as $key => $match) {
            ...
            // var_dump($altrs[1][$key]);
            //file_put_contents("/path/to/images/$uniqueFilename", $emailMessage->attachments[$match]['data']);
            $search[] = "src=\"cid:$match\"";
            //$replace[] = "src=$dir$uniqueFilename";
            $replace[] = "src=".$READ_EMAIL_BODY_IMAGE."?type=attachment&name=".$uniqueFilename."&ym=".$yy.$mm."&day=".$dd."&tid=".$thread_id."&sl=".$mail_id;
        }
        // var_dump($search);
        // var_dump($replace);
...
function generate_log_email_session($thread_id, $session_id, $skill, $from_email,  $conn){
    $now = time();
    $sql = "INSERT INTO log_email_session SET  ticket_id='$thread_id', session_id='".$session_id."', create_time='".$now."', ";
    $sql .= " skill_id='".$skill."', `status`='O',  last_update_time='".$now."', ";
    $sql .= " status_updated_by='".$from_email."' ";
    $sql .= " , customer_email_count=customer_email_count+1 ";
    return $conn->query($sql);
}

function generate_log_email_session_old($thread_id, $conn){
    $sql = "SELECT * FROM e_ticket_info WHERE ticket_id='$thread_id'";
    $result = $conn->query($sql);
    if (is_array($result)){
...
function generate_session_id($ticket_id, $conn){
...
/*function generate_session_id_old($ticket_id, $old_session_id){
...
function is_session_id_exists($ticket_id, $session_id, $conn){
    $sql = "SELECT ticket_id,session_id FROM log_email_session WHERE ticket_id='$ticket_id' AND session_id='$session_id'";
    $result = $conn->query($sql);
    if (is_array($result)) return true;
    return false;
}

function destroy_e_ticket($ticket_id, $conn){
    $sql = "DELETE FROM e_ticket_info WHERE ticket_id='$ticket_id' LIMIT 1";
    return $conn->query($sql);
}
