...
// include_once(APPPATH . "model/Model.php");
// include_once(APPPATH . 'model/MChatLeaveMsg.php');
...
$response = do_cc_request('method=insetChatLeaveMessage&'.$request_str);
...
if(!empty($response)){
	...
	die(json_encode([
		MSG_RESULT => false,
		MSG_TYPE => MSG_ERROR,
		MSG_MSG => 'There are a problem of service request of Chat leave message.'
	]));
}
