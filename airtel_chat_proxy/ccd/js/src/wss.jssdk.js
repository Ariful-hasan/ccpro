...
function chat_login_form(){
	...
	html += "<div class='form-group row'>";
	html += "<label class='col-md-12 greetings-text'>"+greetingsText()+"!!!</label>";
	html += "<div class='col-xs-12 control-label'>Welcome to our LiveChat! Please fill in the form below to chat</div>";
	html += "</div>";
	...
	html += "<label class='col-xs-12 control-label'>I am verified user";
	html += "&nbsp;<span class='chat-form-error'>*</span></label>";
...
function createCustomerChatWindow(left, top, width, height, html) {
	...
	customerWindow += "<div class='col-xs-12'>";
	customerWindow += "<img class='img-thumbnail chat-logo' alt='logo' src='"+baseUrl+"/ccd/image/logo_head_main.png'>";
	customerWindow += "<label class='welcome-text'>Welcome to Live Chat</label>";
	// customerWindow += "<a class='btn btn-primary btn-xs pull-right hide' id='btnChatClose'>Close</a>";
		...
		if(showWebchatHistoryFlag){
			...
			setTimeout(function() {
				var msg = {
					from: agentId,
					name: agentNick,
					msg: "Welcome to the #1 network of friends where you can enjoy the best 4G+ experience, data packs and tariff. To ensure you receive the best service, our customer service agents are now available 24 hours! ",
			...
			setTimeout(function() {
				var msg = {
					from: agentId,
					name: agentNick,
					msg: "This is " + $.cookie("cChatAgentName") + " from Airtel Livechat, How may I help you? ",
					//msg:"আমার নাম " + $.cookie("cChatAgentName"),
					receiverId: $.cookie("cCustomerName"),
					receiverName: $.cookie("cCustomerName")
				};
		...
		$.ajax({
			...
			success:function(resp) {
				...
				if(chat_accept==false){
					...
					body_html += "<img class='img-thumbnail' src='"+baseUrl+"/ccd/images/android.svg' alt='Android'>";
					body_html += "</a>";
					body_html += "</div>";
...
function msgRow(msg, direction) {
	...
	if(direction=="L") {
		row = "<div class='msg-row'>"+
				...
				"<span class='pull-left'><img src='"+baseUrl+"/ccd/image/chat_customer.png' class='img-circle' alt='agent' width='25' height='25'> </span>"+
		...
		row = "<div class='msg-row'>"+
				...
				"<span class='pull-right'><img src='"+baseUrl+"/ccd/image/chat_agent_logo.png' class='img-circle' alt='agent' width='25' height='25'> </span>"+
...
function showRatingView(cid)
{
	...
	ratingWindow += "<div class='col-xs-12'>";
	ratingWindow += "<img class='img-thumbnail chat-logo' alt='logo' src='"+baseUrl+"/ccd/image/logo_head_main.png'>";
	ratingWindow += "<label class='welcome-text'>Welcome to Live Chat</label>";
    ...
    if(document.getElementById("language_en").checked){
        ...
        ratingWindow += "<div class='col-md-12' id='rateYo_caption'>প্রিয় গ্রাহক, এয়ারটেল Webchat থেকে পাওয়া এজেন্ট এর সেবাটি ভালো লাগলে Yes, ভালো না লাগলে No সিলেক্ট করুন</div>";
