...
//$READ_EMAIL_BODY_IMAGE = $extra->read_email_body_image;
$READ_EMAIL_BODY_IMAGE = "http://192.168.10.64/ccprodev/read_email_body_image.php";
$auto_reply_texts = array();
$auto_reply_skill_names = array();

/*********************************Log File Delete**********************************/
$_log_file_path = "/usr/local/ccpro/email/log";
$_log_delete_time = 31*24*60*60; // day's ago log file delete.
$_log_delete_count = 50; // file delete per cron-job.
delete_log();
/***********************************************************************************/
...
$priority_emails_list = !empty($email_settings->priority_email) ? explode(",", $email_settings->priority_email) : '';
$priority_email_skill = !empty($email_settings->priority_email_skill) ? $email_settings->priority_email_skill : '';

$method_list = getFetchDetails($conn);
if (!empty($method_list)) {
    foreach ($method_list as $key){
        if ($key->username == "gplex.cct@robi.com.bd" || $key->username == "gplextest@robi.com.bd")
            continue;
...
function add_new_thread($thread_id, $skill_id, $mail_to, $from_address, $subject, $now, $conn, $customer_id, $fetch_box_name, $fetch_object)
{
    ...
    $sql = "INSERT INTO e_ticket_info SET ticket_id='$thread_id', skill_id='$skill_id', subject='$subject', subject_db='$subjet_for_search', status='O', ".
    ...
    //var_dump($is_update);var_dump($sql);
...
function add_new_mail($mail_meta, $mail_content, $attachment_save_path, $conn, $attachment_directory_owner, $skill_id, $apply_domain_rule, $pop3_user, $fetch_object, $debug, $debug_new_line)
{
    ...
    $from_address['name'] = iconv_mime_decode($from_address['name'], 0, 'UTF-8');
...
function changeImageURL($mail_body, $attachment_save_path, $now, $thread_id, $mail_id, $email_attachment_files, $attachment_name_with_id){
	global $READ_EMAIL_BODY_IMAGE;						   
    $yy = date("y", $now);
    $mm = date("m", $now);
    $dd = date("d", $now);
    $dir = $attachment_save_path . '/' . $yy . $mm . '/' . $dd . '/' . $thread_id . '/' . $mail_id . '/';
    ...
    if(count($matches) > 0) {
        ...
        foreach($matches[1] as $key => $match) {
            //$uniqueFilename = !empty($altrs[1][$key]) ? $altrs[1][$key] : $email_attachment_files[$key]['filename'];
            $uniqueFilename = !empty($attachment_name_with_id[$match]) ? $attachment_name_with_id[$match] : $email_attachment_files[$key]['filename'];
            $search[] = "src=\"cid:$match\"";
			$replace[] = "src=".$READ_EMAIL_BODY_IMAGE."?type=attachment&name=".urlencode($uniqueFilename)."&ym=".$yy.$mm."&day=".$dd."&tid=".$thread_id."&sl=".$mail_id;
        }
...
function save_email_address_book($conn, $customer_id, $name, $email){
    $name = trim($conn->escapeString($name));
...
function get_skill_from_to_address_new($to, $from, $subject, $apply_domain_rule, $conn){
    global $priority_emails_list;
    global $priority_email_skill;
    $toes = explode(",", $to);
    ...
    if (!empty($from)){
        ...
        if (!empty($priority_emails_list) && !empty($priority_email_skill) && in_array($from_address['email'], $priority_emails_list)) {
            return array($priority_email_skill, $to_email);
        } elseif ($apply_domain_rule == 'Y') {
...
function update_log_email_session($conn, $ticket_id){
    ...
    if (!empty($result)){
        ...
        if (!empty($is_exists)){
            ...
            $sql .= " , customer_email_count=customer_email_count+1 ";
...
function delete_log() {
    ...
    global $_log_delete_time;
    global $_log_delete_count;
    $now = time();
    $count = 0;

    $startTime = new DateTime('01:28');
    $endTime = new DateTime('02:00');

    if (date("H:i", $now) >= $startTime->format("H:i") &&  date("H:i", $now) <= $endTime->format("H:i")) {
        $iterator = new DirectoryIterator($_log_file_path);
        foreach ($iterator as $fileinfo) {
            if ($count == $_log_delete_count)
                break;

            if ($fileinfo->isFile() && $now - $fileinfo->getCTime() >= $_log_delete_time) {
