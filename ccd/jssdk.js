...
/*	
	window.onbeforeunload = function() {
		//var newwindow = window.open("http://192.168.10.18/cloudcc/chat_rating.php",'_blank','width=300,height=400');
		//newwindow.focus();
...
*/
	window.onbeforeunload = function(e) {
		...
		*/
		...
		//var newwindow = window.open("http://192.168.10.18/chat_rating.php",'_blank','width='+chat_window_width+',height='+chat_window_height+',left='+left+',top='+top);
		//newwindow.focus();
...
function getForm() {
	...
	var html = "\
		<table>\
			<tr height='30'>\
				<td colspan='2'>Welcome to our LiveChat! Please fill in the form below to chat</td>\
			</tr>\
			<tr height='30'>\
				<td colspan='2' id='gccRequiredFieldErr'></td>\
			</tr>\
			<tr>\
				<td align='right'>Name:</td>\
				<td><input type='text' id='cCustomerName' value='"+name+"'></td>\
			</tr>\
			<tr>\
				<td align='right'>Email:</td>\
				<td><input type='text' id='cCustomerEmail' value='"+email+"'></td>\
			</tr>\
			<tr>\
				<td align='right'>Service:</td>\
				<td><select id='cCustomerSubject'>\
					<option value=''>---Select---</option>";
					for(i=0; i<srv_array.length; i++) {
						var key_title = srv_array[i].split("=");
						html += "<option value='"+key_title[0]+"'>"+key_title[1]+"</option>";
		...
		html += "\
			...
			if(lng_array.length>1) {
				...
				html += "<tr>\
					...
					<td><select id='cCustomerLanguage'>\
						<option value=''>---Select---</option>";
						for(i=0; i<lng_array.length; i++) {
							var key_title = lng_array[i].split("=");
							html += "<option value='"+key_title[0]+"'>"+key_title[1]+"</option>";
				...
				html += "<tr style='display:none;'>\
					...
					<td><select id='cCustomerLanguage'>";
						for(i=0; i<lng_array.length; i++) {
							var key_title = lng_array[i].split("=");
							html += "<option value='"+key_title[0]+"' selected>"+key_title[1]+"</option>";
						}
				html += "\
					</select></td>\
				</tr>";
		...
		html += "<tr>\
				<td align='right'></td>\
				<td><input type='button' value='OK' id='cCustomerInfoSend'></td>\
			</tr>\
		</table>";
		*/
...
(function() {
	...
	loadjscssfile(baseUrl+"/ccd/gchat.css?a", "css");
...
function msgReceived(resp) {
	...
	/*var textReceivedTime = new Date();
	var dif = textReceivedTime.getTime() - textSendTime.getTime();
	var Seconds_from_T1_to_T2 = dif / 1000;
	var Seconds_Between_Dates = Math.abs(Seconds_from_T1_to_T2);
	console.log(textSendTime);
	console.log(textReceivedTime);
	console.log("||||||||"+Seconds_Between_Dates+"||||||||");
	if(Seconds_Between_Dates>15) $('#chatAudio')[0].play();
	*/
	...
	*/
	...
	var item = { from:senderId, name:senderNick, msg:atob(resp.message) };
...
function chatWriting() {
	...
	if($typingIndicator.length<=0) {
		...
		$box.append("<div id='gccw-typing'>Agent is typing...</div>");
	}
}
...
function countUpTextTypingTime() {
	...
	//for(user_id in chatBoxes) {
		...
		var startTime = new Date($.cookie("textTypingAt"));
...
{
	console.log("Pinging...");
	var data = btoa(JSON.stringify({isTyping:true}));
	obj.Reg(data);
...
function showRatingView(cid)
{
	...
	ratingWindow += "<div id='rateYo'></div>";
	ratingWindow += "<div id='rating' class='col-md-12' style='height:30px; font-weight:bold; color:#bbbbbb;'>&nbsp;</div>";
