var gPlexCCWSApi = (function() {
	...
	function CallBackResponse(msg) {
		...
		switch(method) {
			...
			case "CALL":
				...
				if (!isInCall) currentCallId = resp_ary[6];
				var cWay = resp_ary[1];
				var cDid = resp_ary[2];
				...
				if(cWay=="IN") notifyMe("Calling...", "You have a call from "+callerid);
				...
				if(resp_ary.length>9) {
					...
					var url = resp_ary[10];
					//debug("URL: "+url);
					//if (typeof url != 'undefined' && url.length>0) IframeOpen(url);
					...
					if (typeof url != 'undefined' && url.length>0) {
						...
						if(url.indexOf("&param=") >=0 ) {
							...
							if(cSkill.length>0) {
								...
								var o = result[0];
								templateArr["skill_name"] = o.skill_name;
							}
						...
						//console.log(url);
						IframeOpen(resp_ary[6], callerid, url);
					}
				}
			...
			case "CHANGE_AUX":
				...
				if(resp_ary[1]=="200 OK") {
					...
					//console.log(selectedAuxId);
					$.cookie(cookieVars.selectedAuxId,cookieValue.selectedAuxId);
					//console.log($.cookie("wsCCSelectedAuxId"));
					if(resp_ary[2] != "0") {
						if(resp_ary[2] == "21") {
							$("#btnAux").addClass("busy").removeClass("primary").removeClass("active").html("Missed Call");
						} else {
							var txt = "Busy";
							try {
			...
			case "CHAT_MONITOR":
				...
				break;
			case "CHAT_CLOSE":
				_Chat.ChatClose(resp.call_id, true);
				break;
	...
	gPlexCC.prototype.loadSettings = function(url, ag, pass) {
		uri = url;
		agentId = ag;
		token = pass;
	};
