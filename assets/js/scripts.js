...
jQuery(function($) {
    ...
    ULTRA_SETTINGS.isResizing=false;
    ...
    ULTRA_SETTINGS.windowBasedLayout = function() {
        ...
        if ($("body").hasClass("chat-open") || $("body").hasClass("sidebar-collapse")) {
        	...
        	if(!$(".page-topbar").hasClass("force-collpse")){ 
	            // large window
    ...
    ULTRA_SETTINGS.chatAPI = function() {
        ...
        $('.page-topbar .sidebar_toggle').on('click', function() {
            ...
            if (menuarea.hasClass("collapseit") || menuarea.hasClass("chat_shift")) {
                ...
                chatwindow.addClass("hideit").removeClass("showit");
                ULTRA_SETTINGS.mainmenuScroll();
                topbar.removeClass("force-collpse");
                detCookie("mcl");
            } else {
    ...
    /* added by Sarwar*/
    ULTRA_SETTINGS.SetMenuCollapse= function() {
        try{
        	  var chatarea = $(".page-chatapi");
              var chatwindow = $(".chatapi-windows");
              var topbar = $(".page-topbar");
    ...
    ULTRA_SETTINGS.Resize=function(){
    	   if(!ULTRA_SETTINGS.isResizing){
        	   ULTRA_SETTINGS.isResizing=true;
        	   setTimeout(function(){        		  	  
        		  try{
        			  ResizeAll();
        		  }catch(e){ }
    ...
    $(document).ready(function() {
        ...
        ULTRA_SETTINGS.SetMenuCollapse();
    });
...
function setDateFormat(onchange_element = '#date_format', field = '.gs-report-date-picker') {
    $(onchange_element).on('change', function(){
        var df = $(this).val();
        var current_date = '';
        ...
        if(df == 'd/m/Y'){
            default_date = current_date.getDate()+'/'+(+current_date.getMonth()+1)+'/'+current_date.getFullYear();
        }else if(df == 'm/d/Y'){
            default_date = (+current_date.getMonth()+1)+'/'+current_date.getDate()+'/'+current_date.getFullYear();
        }
    ...
    //$(onchange_element).change();
}

function setChartDateFormat(format, sdate, edate) {
    date_format = format;

    try{
        $(".from-date").each(function(e){
...
function getServiceIdDropDown() {
    $("#service_id").on('change', function(){
        ...
        if(val){
            option_str = '<option value="*">All</option>';
            if(typeof service_id[val][skill_type]!='undefined'){
                $.each(service_id[val][skill_type], function(idx, item){
                    option_str += '<option value="'+idx+'">'+item+'</option>'; 
                });
            }else{
                bootbox.alert({
                    message: "There are no skill available!",
                    backdrop: true,
                    callback: function (e) {
                        // console.log(e);
                        $('#service_id').select2("val", "");
            ...
            option_str = '<option value="*">All</option>';
            $.each(skill_list[skill_type], function(idx, item){
                option_str += '<option value="'+idx+'">'+item+'</option>'; 
            });
        }
        $('#skill_id').html(option_str);
...
function beforeFormSubmit(day){
    ...
    if ((d2.getTime() - d.getTime()) / (1000*60*60*day*24) > 1) {
        var alert_m = 'Provide date range within '+day;
        alert_m = (day > 1) ? alert_m+' days' : alert_m+' day';
        alert(alert_m);
        return false;
    }
...
function beforeFormSubmitEmail(day, ssrc, esrc, label){
    var start_date = $(ssrc).val();
    var end_date = $(esrc).val();
    ...
    // console.log(1000*60*60*parseInt(day)*24);
    if ((d2.getTime() - d.getTime()) / (1000*60*60*day*24) > 1) {
        var alert_m = 'Provide date range ('+label+') within '+day;
        alert_m = (day > 1) ? alert_m+' days' : alert_m+' day';
        alert(alert_m);
        return false;
    }
