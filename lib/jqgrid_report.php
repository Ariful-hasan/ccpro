...
class jQGridReport{
	...
	var $colModel=array();	
	var	$rowNum=15; 
	var $rowList=array(5,10,20,50,100,200); 
	var $mtype="POST";
	var	$caption=null;	
	var	$rownumbers= true;
  	var	$pager=''; 
	...
	var $postData=null;
	var $CustomSearchOnTopGrid=true;
	var $ShowDefaultSearch=false;
	var $emptySetText="No Record Found";
	var $TotalColumn=0;	
	var $isShowNoRecordMsg=true;
	var $IsColAutoWidth=false;
	var $minWidth=200;
	var $rightPadding=0;
	var $ShowReloadButtonInTitle=false;
	var $ShowDownloadButtonInBottom=false;
	var $ShowDownloadButtonInTitle=false;
	var $ShowSMSBtnWithReload = false;
	var $DownloadFileName="";
	var $TextwDownloadButtonInTitle="Download CSV";
	var $TextReloadButtonInTitle="Reload";
	/* Toolbar*/
	var $toolbar=array(false,"top");
	var $toolbarControl="";
	var $toolbarHeight=null;
	var $toolbarCSS="";	
	/*end toolbar*/
	
	var $isForBootstrap=true;
	...
	/******************** Grouping grid data based on  column value*******************/
	private $grouping = FALSE;
	private $groupingField = [];
	private $groupColumnShow = FALSE;
	private $showGroupSummery = FALSE;
	private $groupCollapse = FALSE;
	/*----------Header column Group /Span ------------*/
	public $headerColumnGroups = [];
	/*----------Show userData with main rows-------- */
    public $showUserDataWithMainRows = false;	
    public $DateRange = SUMMARY_REPORT_DAY; /*----------Show data by date range-------- */
    public $EmailMultipleDate = false; /*----------Show data by date range-------- */

    public $floatingScrollBar=false; /*Floating scroll bar 21/07/19*/

  	function __construct($gid=""){
  		
  		if(function_exists("CheckAdminSession")){  			
  			$this->ShowDownloadButtonInTitle=CheckAdminSession();  			
  		}
	...
	function show($srcJquerySelector=""){
		...
		if($this->IsColAutoWidth){
		 ...
		 if(!empty($this->ShowReloadButtonInTitle) || !empty($this->ShowDownloadButtonInTitle)){
			 if ($this->ShowSMSBtnWithReload){
				 $reload = $this->ShowReloadButtonInTitle?'<a onclick="'.($this->ReloadMethod()).'()" class="btn btn-xs btn-purple" ><i class="fa fa-refresh"></i> '.$this->TextReloadButtonInTitle.'</a> ':"";
				 $reload .= ' &nbsp; <a id="send_sms_btn" class="btn btn-xs btn-info"> Send SMS </a>';
			 }else {
				 $reload=$this->ShowReloadButtonInTitle?'<a onclick="'.($this->ReloadMethod()).'()" class="btn btn-xs btn-purple" ><i class="fa fa-refresh"></i> '.$this->TextReloadButtonInTitle.'</a> ':"";
			 }
		 	//$reload=$this->ShowReloadButtonInTitle?'<a onclick="'.($this->ReloadMethod()).'()" class="btn btn-xs btn-purple" ><i class="fa fa-refresh"></i> '.$this->TextReloadButtonInTitle.'</a> ':"";
			...
			$this->caption.='</span><span style="margin-left: 20px; float: left;" class="gridtitle left">'.$reload.'</span>';
...
<script type="text/javascript">
		 	...
		 	//custom code to search when press enter key
			...
			jQuery("#autosearch_<?php echo $tableId;?>").click(function(){
			 ...
			 jQuery(".gs-jq-grid").on("click",".full-screen:not(.exit-full-screen)",function(e){
				 ...
				 jQuery("#<?php echo $tableId;?>").setGridWidth(wwidth+5);
			...
			<?php if($this->CustomSearchOnTopGrid){?>
		 	...
		 	function <?php echo $this->DownloadCSVMethod()?>(){
		 	    document.cookie = 'download_csv=0';
                showProcessing('<i class="fa fa-spinner fa-spin" style="font-size:47px"></i> Processing...');
			 	...
			 	// After download stat hide processing message

                var downloadTimer = window.setInterval( function() {
                    var token = getCookie( "download_csv" );
                    if (token === '1'){
                        $("#MainLoader").hide();
                        delete_cookie('download_csv');
            ...
            function delete_cookie( name ) {
		 	...
		 	function Grid_<?php echo $tableId?>_custom_reload(){
		 	    try {
                    <?php if($this->EmailMultipleDate){ ?>
                    	// for only email
	                    if(!beforeFormSubmitEmail(<?php echo $this->DateRange; ?>, "input[name='ms[create_time][from]']", "input[name='ms[create_time][to]']", "Arrival Date\Time")){
	                    	return;
		                }
		                if(!beforeFormSubmitEmail(<?php echo $this->DateRange; ?>, "input[name='ms[first_open_time][from]']", "input[name='ms[first_open_time][to]']", "First Open Time"))
		                {
		                    return;
		                }
		                if(!beforeFormSubmitEmail(<?php echo $this->DateRange; ?>, "input[name='ms[close_time][from]']", "input[name='ms[close_time][to]']", "Closed Time"))
		                {
		                    return;
		                }
		                if(!beforeFormSubmitEmail(<?php echo $this->DateRange; ?>, "input[name='ms[last_update_time][from]']", "input[name='ms[last_update_time][to]']", "Open Time")){
		                    return;
		                }
		            <?php }else{ ?>
		                if(!beforeFormSubmit(<?php echo $this->DateRange; ?>)){
	                        return;
	                    }
	                <?php } ?>
                }catch(e){
                ...
                //$dropdown.trigger('change');
		 		...
		 		var data = jQuery("<?php echo $this->GetGridId();?>").jqGrid("getGridParam", "postData");				
		 		data.first=true; //data.download_csv = true;
		 	...
		 	function Grid_<?php echo $tableId?>_reset_custom_reload(){
                ...
                var $dropdown = jQuery(".select2");
                var $skill_type =  jQuery("#skill_type");
                ...
                try{
                    ...
                    if ($skill_type.length){
                        $skill_type.select2("val", "V");
		...
		jQuery(function($){
			...
			<?php if ($this->footerRow): ?>
                config_<?php echo $tableId?>.footerrow = true;
                config_<?php echo $tableId?>.userDataOnFooter = true;
                var footerData = <?php echo !empty($this->jsonReader->userdata) ? $this->jsonReader->userdata :  ""; ?>
                jQuery("#<?php echo $tableId?>").footerData('set',footerData, false);
                jQuery(".footrow td" ).css({'font-weight':'bolder'});
            <?php endif;?>

            <?php if ($this->grouping): ?>
            <?php
                $groupConfiguration = new stdClass();
                $groupConfiguration->groupField = $this->groupingField;
                $groupConfiguration->groupColumnShow[] = $this->groupColumnShow;
                $groupConfiguration->groupSummary[] = $this->showGroupSummery;
                $groupConfiguration->groupCollapse = $this->groupCollapse;
            ?>
                config_<?php echo $tableId?>.grouping = true;
                config_<?php echo $tableId?>.groupingView  = <?php echo json_encode($groupConfiguration); ?>;

            <?php endif;  ?>

            config_<?php echo $tableId?>.afterInsertRow=eval(config_<?php echo $tableId?>.afterInsertRow);
			config_<?php echo $tableId?>.loadComplete=function(e){
				//---------------show/hide col	------------------------------
				if(e.hideCol.length > 0){
					$.each(e.hideCol, function(idx, item){
						jQuery("<?php echo $this->GetGridId();?>").jqGrid('hideCol', item);
					});					
				}

				if(e.showCol.length > 0){
					$.each(e.showCol, function(idx, item){
						jQuery("<?php echo $this->GetGridId();?>").jqGrid('showCol', item);
					});					
				}

                SetLightBox();<?php echo $this->loadComplete?>(e);
                <?php if($this->styleLastRow) : ?>
                    lastRow = jQuery("#<?php echo $tableId?> tr:last").attr("id");
			        jQuery("#"+lastRow+" td" ).css({'font-weight':'bold'});
			        jQuery("#"+lastRow).addClass('ui-state-default ui-th-column ui-th-ltr');
			    <?php endif; ?>
			...
			jQuery("#gview_<?php echo $tableId;?> .ui-jqgrid-bdiv .gridnorecord").hide().remove();
			 if (jQuery("#<?php echo $tableId;?>").getGridParam("records")==0) {
                 ...
                 try {jQuery(".footrow" ).hide();}catch(e){}
             }else{
                 try {jQuery(".footrow" ).show();}catch(e){}
             }
			 <?php }?>
			};
            ...
            /*------------------- Header Column Group Functionality Add Start ------------*/
            jQuery("#<?php echo $tableId;?>").jqGrid('setGroupHeaders', {
                useColSpanStyle: true,
                groupHeaders:<?php echo $this->getHeaderColumnGroups(); ?>
            });
            /*-----------------Fix header column group height and alignment ------------*/
          //  $('.ui-jqgrid-sortable').css({'padding':'0px 2px'});
            $('.ui-th-column-header').css({'text-align':'center','height':'35px'});
            /*------------------- Header Column Group Functionality Add End ------------*/
			...
			jQuery(window).bind('resize', function() {
				//console.log(jQuery("#<?php echo $tableId;?>").parents(".gs-jq-grid "));
				var c_minwidth_<?php echo $tableId;?>=jQuery("#<?php echo $tableId;?>").parents(".gs-jq-grid ").width();//jQuery("<?php echo $this->container;?>").width();
            ...
            <?php if ($this->floatingScrollBar) {?>
                setFloatinScrollBar();
            <?php }?>
				
		});
        function setFloatinScrollBar(){
            $(".ui-jqgrid-bdiv").addClass('overflow-panel');
...
<div id="mc_<?php echo $tableId?>" class="gs-jq-grid ">
	<form id="grid_form_data" name="grid_form_data" method="post" action="">
		<table id="<?php echo $tableId?>"></table>
		<div id="<?php echo "pager_".$tableId;?>"></div>
		<input type="hidden" name="last_sms_text" id="last_sms_text" value="">
		<input type="hidden" name="ssms_template" id="ssms_template" value="">
	</form>
</div>
...
<?php 
	...
	function AddSearhProperty($title,$index_id,$search_type="",$searctionObtionValue=array()){
    ...
    function AddDownloadHiddenProperty($index_id){
        //$this->AddModel($title, $index_id, $width,$align,"","","",true,$search_type,$searctionObtionValue,$isSortable);
        $this->AddCustomModel(
            array(  "Title"=>$index_id,
                "search"=>false,
    ...
    public function addHeaderColumnGroup($startColumnName,$numberOfColumns,$titleText)
    {
        $this->headerColumnGroups[] = [
    ...
    public function addGroup($groupingField,$groupColumnShow=FALSE,$groupCollapse=FALSE)
    {
        $this->grouping = TRUE;
        $this->groupingField[] = $groupingField;
        $this->groupColumnShow = $groupColumnShow;
        $this->showGroupSummery = FALSE;
        $this->groupCollapse = $groupCollapse;
    }

    public function getHeaderColumnGroups()
    {
        return json_encode($this->headerColumnGroups);
    }
...
<div class="gs-grid-serach row form-horizontal" id="<?php echo $srcDivId;?>"	style="padding: 5px;">
	<div class="col-xs-4 ">
		<div class="form-group">
			...
			<select class="input-sm srcOptionList form-control select2">
			 	<?php 
			 	$already=array();
		...
		<div class="form-group">
			...
			<div class="col-sm-6">			
				<input autocomplete="off" class="srcTextValue form-control input-sm"
					...
					class="input-sm srcSelectValue form-control select2" style="display: none;"
					name="srcText">					
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<div class="input-group date gs-date-picker-grid-options">
					<input autocomplete="off" class="gs-report-date-picker input-xs srcTextValue form-control srcFrom input-sm" type="text" name="srcFrom" value="" />
	...
	<div id="<?php echo $srcDivId."_to"?>" class="col-xs-4 hidden">
		<div class="form-group">
			...
			<div class="col-sm-10">
				...
				<input autocomplete="off" class="gs-report-date-picker srcTextValue form-control  srcTo input-sm"  	type="text" name="srcTo" value="" />
				<span class="input-group-addon" style="height: 24px !important; padding:6px 3px 0px 1px!important; line-height: 4px !important;"><span style="font-size: 8px !important;" class="fa fa-calendar"></span></span>
				</div>
			</div>
		</div>
				  		...
				  		$datatest="";
							...
							<div class="form-group ">
				      		    ...
				      		    <div class="col-sm-10 col-md-8">	
				      		 	...
				      		 	<?php }elseif(strtolower($model->stype)=="date"||strtolower($model->stype)=="dateonly" || strtolower($model->stype)=="datetime" || strtolower($model->stype)=="report-datetime"){
				      		 		...
				      		 		if( strtolower($model->stype)=="report-datetime"){
                                        $model->dvalue=!empty($model->dvalue)?$model->dvalue:"";
                                        $model->dvalue2=!empty($model->dvalue2)?$model->dvalue2:"";
				      		 		}
				      		 	?>
				      		 	 <div class="col-sm-<?php echo $maintype=="dateonly"?10:5;?> col-md-<?php echo $maintype=="dateonly"?8:4;?>">
				      		 		 <div class="input-group <?php echo strtolower($model->stype)?> <?php echo strtolower($model->stype)."-to"?> <?php if(empty($custom)){?>gs-<?php echo strtolower($model->stype)."-from";?>-picker-grid-options"<?php }else{?>gs-<?php echo strtolower($model->stype).$custom;?>-picker-grid-options<?php }?>" >
				      		 	 ...
				      		 	 <div class="col-sm-5 col-md-4">
				      		 	 	 <div class="input-group <?php echo strtolower($model->stype)?>  <?php if(empty($custom)){?>gs-<?php echo strtolower($model->stype)."-to";?>-picker-grid-options"<?php }else{?>gs-<?php echo strtolower($model->stype).$custom;?>-picker-grid-options<?php }?>" >
										<input autocomplete="off" class="srcTextValue form-control srcMTo" 	type="text" name="ms[<?php echo $model->name;?>][to]" value="<?php echo $model->dvalue2;?>"  placeholder="To"   <?php echo !empty($custom) ? ' data-format="'.$model->ExtraParam.'"' :'';?>  />
										<span class="input-group-addon" style="height: 24px !important; padding:2px 2px 0px 1px ; line-height: 4px !important;"><span style="font-size: 12px !important;" class="fa <?php echo strtolower($model->stype)=="date"?' fa-calendar ':' fa-clock-o '?>"></span></span>										
									</div>
				      		 	 </div>
				      		 	 <?php }?>
				      		 	...
				      		 	?>
				      		 	 	<div class="col-sm-10 col-md-8">				      		 	 					      		    	
				      		    		<select class="form-control input-sm select2" id="<?php echo $model->name;?>" name="ms[<?php echo $model->name;?>]">
				      		    			<?php
				      		    			if(!empty($model->searchoptions->value) && count($model->searchoptions->value)>0){											
