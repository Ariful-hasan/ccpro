...
class jQGrid{
    ...
    var $searchID="";
    var $resetsrcID="";
    var $isDestroyCOOKIE=true;
    var $CookieArray="";
    var $isForBootstrap=true;
    var $horizontalTopScrollBar=false;
    var $floatingScrollBar=false;
...
<script type="text/javascript">
			...
			<?php if($this->CustomSearchOnTopGrid){?>
		 	...
		 	function Grid_<?php echo $tableId?>_reset_custom_reload(isDestroyCOOKIE='', destroyedCookieArray=''){
		 	    if ((isDestroyCOOKIE.length >0 && isDestroyCOOKIE==1) && (typeof destroyedCookieArray !== 'undefined' && destroyedCookieArray!=null && destroyedCookieArray.length > 0)){
		 	        destroyCookie(destroyedCookieArray);
                }
		 		jQuery("#ms_<?php echo $tableId;?>")[0].reset();
		 		Grid_<?php echo $tableId?>_custom_reload();
		 	}
		 	function destroyCookie(destroyedCookieArray){
                destroyedCookieArray = destroyedCookieArray.split(',');
		 	    console.log(destroyedCookieArray);
                $.each(destroyedCookieArray, function(idx, val){
                    document.cookie = val + "=''";
                })
            }
		...
		jQuery(function($){
          ...
          //  $('.ui-jqgrid-sortable').css({'padding':'0px 2px'});
            ...
            <?php if ($this->horizontalTopScrollBar) {?>
                setHorizntalTopScrollBar();
            <?php } ?>
        ...
        function setHorizntalTopScrollBar(){
            var tableContainer = $(".ui-jqgrid-bdiv");
            var table = $(".ui-jqgrid-bdiv table");
...
<?php 
	...
	function AddSortableModel($title,$index_id,$width ,$align="left",$formatter="",$srcformat="",$newformat="",$isSearchable=true,$stype="",$serchoption="",$isSortable=true){
  	...
  	function IsDataExists($index_id){
        return $this->tempIndex[$index_id];
    }
  	/* for Toolbar*/
...
<div class="gs-grid-serach row form-horizontal" id="<?php echo $srcDivId;?>"	style="padding: 5px;">
	...
	<div class="col-xs-4">
		...
		<a class="btn btn-xs btn-warning"
			...
			<a class="btn btn-xs btn-danger" onclick="javascript:Grid_<?php echo $tableId?>_reset_custom_reload('<?php echo $this->isDestroyCOOKIE?>', '<?php echo $this->CookieArray?>');"><i class="fa fa-times"> </i> <?php _e("Reset") ; ?></a>
		...
		<div class="col-xs-12 ">		
			<div class="panel panel-default">
			  <div class="src-heading panel-heading"><?php _e("Search"); ?>			  	
			  	 <a class="pull-right btn btn-xs btn-warning" <?php if ($this->searchID) echo "id='$this->searchID'";?> onclick="javascript:Grid_<?php echo $tableId?>_custom_reload();"><i class="fa fa-search"> </i> <?php _e("Search") ; ?></a>
			  	 <a class="pull-right btn btn-xs btn-danger" <?php if ($this->resetsrcID) echo "id='$this->resetsrcID'";?> onclick="javascript:Grid_<?php echo $tableId?>_reset_custom_reload('<?php echo $this->isDestroyCOOKIE?>', '<?php echo $this->CookieArray?>');"><i class="fa fa-times"> </i> <?php _e("Reset Search") ; ?></a>
			  </div>
