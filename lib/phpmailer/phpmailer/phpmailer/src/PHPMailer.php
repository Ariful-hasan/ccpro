...
class PHPMailer
{
     ...
     */
    public function send($conn=null, $mailboj=null)
    {
        /*
         * here db object pass manually for email
         * but it's not default in phpmailer
         * $mailboj also manual.
         */
        try {
            if (!$this->preSend()) {
            ...
            /*
             * Manually added to log SMTP ERROR
             */
            if (!empty($conn) && !empty($mailboj)){
                ...
                $conn->query($sql);
                file_put_contents('/usr/local/ccpro/email/log/temp/smtp/'. $mailboj->ticket_id . '_' . $mailboj->mail_sl .'_smtp.log', print_r($this->ErrorInfo, true));
            }
