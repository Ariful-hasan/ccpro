...
$sql = "SELECT * FROM emailsendmodule_out where `to` !='' AND status='' ORDER BY tstamp LIMIT 30";
...
//    $mail->WordWrap   = 80;
    ...
    foreach ($result as $mail_details) {
        $mail_details->to = trim($mail_details->to);

        if (!empty($mail_details->to)) {
            $from_email = !empty($mail_details->from) ? $mail_details->from : $from_email;
            $from_name = $from_name;

            $mail->ClearAllRecipients();
            $mail->ClearReplyTos();
            $mail->ClearAttachments();
            $to_emails = $mail_details->to;
            ...
            $mail->Subject = base64_decode($mail_details->subject);
            ...
            if ($mail_details->has_attachment) {
                $sql = "SELECT * FROM emailsendmodule_attachment WHERE email_id='$mail_details->email_id' ORDER BY part_position";
                ...
                if (is_array($attachments)) {
                    ...
                    $dir = $attachment_save_path . '/' . $yy . $mm . '/' . $dd . '/' . $mail_details->email_id . '/';
            ...
            if ($is_email_found) {
                try {
                    if ( !$mail->Send($conn, $mail_details) ) {
                        ...
                        $sql_update = "DELETE FROM emailsendmodule_out WHERE email_id='$mail_details->email_id'";
                        $result = $conn->query($sql_update);
...
function updateStatus($conn, $mailboj, $error_info){
    if (!empty($conn) && !empty($mailboj)){
        $sql = "UPDATE emailsendmodule_out SET `status`='E' WHERE email_id='$mailboj->email_id' LIMIT 1;";
        $conn->query($sql);
        file_put_contents('/usr/local/ccpro/email/log/temp/smtp/'. $mailboj->email_id .'_smtp.log', print_r($error_info, true));
    }
}
