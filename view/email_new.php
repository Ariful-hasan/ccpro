...
$grid->CookieArray = 'create_time,ticket_id,customer_id,last_name,from_email,subject,did,phone,skill_name,mail_to';
...
$grid->AddModel("Email Address", "from_email", 150,"center");
$grid->SetDefaultValue("from_email", $_COOKIE['from_email']);
...
$grid->AddModel("Last Pull Time", "last_pull_time", 130, "center", ".date", "Y-m-d H:i:s", "Y-m-d H:i:s", true, "datetime");
$grid->SetDefaultValue("last_pull_time", $_COOKIE['lut_sdate'], $_COOKIE['lut_edate']);
...
$grid->AddModelNonSearchable("Last Serial", "mail_sl", 100,"center");
...
<script type="text/javascript">
    ...
    var lut_info = {sdate:$('input[name="ms[last_pull_time][from]"]').val(), edate:$('input[name="ms[last_pull_time][to]"]').val()};
    var create_info = {sdate:$('input[name="ms[create_time][from]"]').val(), edate:$('input[name="ms[create_time][to]"]').val()};
    $(document).ready(function() {
        ...
        $("#src_all_emails").on('click', function () {
            ...
            var lut_sdate = $('input[name="ms[last_pull_time][from]"]').val();
            setCookie("lut_sdate", lut_sdate);
            var lut_edate =$('input[name="ms[last_pull_time][to]"]').val();
            setCookie("lut_edate", lut_edate);

            var from_email = $("#from_email").val();
            setCookie("from_email", from_email);
        ...
        $("#rst_all_emails").on('click', function () {
            $('input[name="ms[create_time][from]"]').val("<?php echo date('Y-m-d H:i', strtotime('-2 month'))?>");
            $('input[name="ms[create_time][to]"]').val("<?php echo date('Y-m-d H:i')?>");
            $("#customer_id").val('');
            $("#from_email").val('');
            $("#did").val('');
            $("#phone").val('');
            ...
            //$('input[name="ms[last_pull_time][from]"]').val("<?php //echo date('Y-m-d H:i', strtotime('-2 month'))?>//");
            //$('input[name="ms[last_pull_time][to]"]').val("<?php //echo date('Y-m-d H:i')?>//");
            $('input[name="ms[last_pull_time][from]"]').val("");
            $('input[name="ms[last_pull_time][to]"]').val("");


            // setCookie("crt_sdate", '');
            // setCookie("crt_edate", '');

            setCookie("crt_sdate", "<?php echo date('Y-m-d H:i', strtotime('-2 month'))?>");
            setCookie("crt_edate", "<?php echo date('Y-m-d H:i')?>");
        ...
        lut_info = {sdate:$('input[name="ms[last_pull_time][from]"]').val(), edate:$('input[name="ms[last_pull_time][to]"]').val()};
        create_info = {sdate:$('input[name="ms[create_time][from]"]').val(), edate:$('input[name="ms[create_time][to]"]').val()};
    ...
    // function autoRefreshTopMenu() {
    //     setInterval(function () {
    //         updateTopMenuCount();
    //     },30000);
    // }
    function updateTopMenuCount() {
        if (typeof parsed_menu_ids !== 'undefined'){
            $.ajax({
                dataType    :   "JSON",
                type        :   "POST",
                url         :   "<?php echo site_url().$this->url('task=email-new&act=get-topmenu-count') ?>",
                data        :   {email: menu_emails, create_info: create_info},
                success     :   function (res) {
