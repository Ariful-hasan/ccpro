...
<?php if (!empty($errMsg)):?><br /><?php if ($errType === 0):?><div class="alert alert-success"><?php else:?><div class="alert alert-error"><?php endif;?> <?php echo $errMsg;?></div><?php endif;?>
                    ...
                    <div class="col-md-12">
                        <div class="panel panel-default">
                            <div class="panel-body">
                                <div class="col-md-12 hide" id="count_div">
                                    ...
                                    <div class="col-md-11">
                                        ...
                                        <div class="col-md-2 mt-10">
                                            <button type="button" class="btn btn-lg btn-warning" onclick="refreshPage()"><i class="fa fa-refresh" aria-hidden="true"></i> Refresh</button>
                                        </div>
                                    </div>
                                ...
                                <div class="col-md-4" id="delete_div">
                                    <div class="panle panel-info common-bdr">
    ...
    <script>
        var PD_SETTINGS_HEADER = <?php echo json_encode($pd_settings)?>;
        var upload_div = $("#upload_div");
        var file = undefined;
        const urlParams = new URLSearchParams(window.location.search);
        var isDelete = "&is_delete=";
        ...
        var bind_ids = [];
        var upload_url = "<?php echo site_url().$this->url("task=" . $request->getControllerName() . '&act=file-upload'.'&skill_id='.$skill_id); ?>";

        const APPENDED_VALUES = [];
        var FILE_NAME = "pd_file";
        var is_uploaded = "";
        var upload_number_url = "<?php echo site_url().$this->url('task=' . $request->getControllerName() . "&act=upload-number&skill_id=".$skill_id)?>";
        upload_number_url += isDelete;
        let refreshPage = () => {
        ...
        $('#is_delete').on('click', function (e) {
            if ($(this).is(":checked")) {
                upload_number_url = upload_number_url.replace('is_delete=', 'is_delete=Y')
            } else {
                upload_number_url = upload_number_url.replace('is_delete=Y', 'is_delete=')
            }
        });
    </script>