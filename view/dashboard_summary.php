...
<div class="container-fluid" id="dashboard">
  <div class="header-summary row">
        ...
        <div class="col-xs-8 col-sm-9 col-md-9 col-lg-9">
            <h3 class="text-center text-info"><?php echo !empty($dashboard_title) ? $dashboard_title : "" ?></h3>
        </div>
...
<script>
    ...
    var dashboard_agent_with_skill_set = <?php echo !empty($dashboard_agent_with_skill_set) ? json_encode($dashboard_agent_with_skill_set) : json_encode(array()); ?>;

    var dashboard_skill = <?php echo !empty($dashboard_skills) ? json_encode($dashboard_skills) : json_encode(array()); ?>;
    ...
    var r = "<?php echo !empty($request->getRequest('r', '')) ? $request->getRequest('r', '') : ''; ?>";
...
<script>
    function isDashboardAgent(skill_list = []){
        let isDashboardSkill = skill_list.filter(function (s) {
            return dashboard_skill.indexOf(s) !== -1;
        });

        return isDashboardSkill.length > 0;
    }
    ...
    const _DashBoard =  {
        ...
        PopulateSkill: function(skill_id, skill) {
            ...
            if(dashboard_skill.includes(skill_id)){

                let calls_in_queue = skill.ciq_call_count_0 +  skill.ciq_call_count_1;
                Vue.set(app.skills[skill_id], 'calls_in_queue', calls_in_queue);

                // Find out total Calls in Queue...
                let total_calls_in_queue =  app.sortedSkills.reduce( function(a, b){
                    let calls_in_queue = 0;
                    if (dashboard_skill.includes(skill_id) && b.hasOwnProperty('calls_in_queue')){
        ...
        PopulateSeat: function(seatId, seat) {
            ...
            let status = seat.status === "B" ? app.aux_messages[seat.aux_code].message : app.seat_status[seat.status];
    ...
    var app = new Vue({
        ...
        data : {
            ...
            agents : _.keyBy(dashboard_agent_with_skill_set,'agent_id'),
            ...
            loggedAgents: function () {
                return _.filter(this.seats, function (a,b) {
                    return a.hasOwnProperty('seat_id')
                        ...
                        && a.hasOwnProperty('skillSet') && isDashboardAgent(a.skillSet);
                });
            }
        },
        methods:{
            getAverageHandlingTime: function () {
                axios.get('<?php echo $this->url('task=aht&act=get-by-skills'); ?>', {params: {
                        r: r
                    }})
