...
<script type="text/javascript">
    ...
    let websocket = '';
    let email_info = {email_id:email_id, agent_id:agent_id, skill_id:"<?php echo $eTickets->skill_id?>"};
    let conn_sock = '';
    ...
    $(document).ready(function() {
        ...
        <?php if (!empty($is_setinterval_active)) { ?>
            ...
            // websocket = new WebSocket("ws://192.168.10.64:8090");
            // websocket.onopen = (event) => {ws_send("PING")};
    ...
    let setCcEmails = (address_book_emails) => {
        set_CC_BCC_FWD('input#email_to', to_tags_email_arr, '', address_book_emails);
        set_CC_BCC_FWD('input#email_cc', cc_tags_email_arr, '#cc_tags_email_arr', address_book_emails);
        set_CC_BCC_FWD('input#email_bcc', bcc_tags_email_arr, '#bcc_tags_email_arr', address_book_emails);
        set_CC_BCC_FWD('input#forward', fwd_tags_email_arr, '#fwd_tags_email_arr', address_book_emails);
    ...
    let getCcEmails = () => {
        let addressbook_count = localStorage.getItem('addressbook_count') || 0;
        let addressbook = localStorage.getItem("addressbook");
        //localStorage.removeItem('addressbook_count');
        $.ajax({
            dataType : "JSON",
            type     : "POST",
            url      : "<?php echo site_url().$this->url('task=email&act=get-email-addressbook');?>",
            data     : {total: addressbook_count},
            success  : function (res) {
                if (typeof res.status !== 'undefined' && res.status === true && typeof res.data !== 'undefined' && res.data.length > 0){
                    setCcEmails(res.data);
                    localStorage.setItem('addressbook_count', res.count);
                    localStorage.setItem('addressbook', JSON.stringify(res.data));
                } else {
                    console.log('local storeage data');
                    setCcEmails(JSON.parse(addressbook));
                }
            }
        });
    };
    ...
    function ws_send(method){
        if (websocket != ''){
            email_info.method = method;
            websocket.send(JSON.stringify(email_info));
        }
    }
    function agentPingForDahsboard() {
        $.ajax({
            dataType: "JSON",
            type:     "POST",
            url:      "<?php echo site_url().$this->url('task=email&act=agent-ping-for-email-dashboard');?>",
            data:     {method:"PING", email_id:email_id, agent_id:agent_id},
            success : function (res) {
                console.log( res);
            }
        });
    ...
    function skipEmail(is_setinterval_active, evt_type='') {
        ...
        $.ajax({
            dataType    :   "JSON",
            type        :   "POST",
            url         :   "<?php echo site_url().$this->url('task=email&act=email-module-udp-msg') ?>",
