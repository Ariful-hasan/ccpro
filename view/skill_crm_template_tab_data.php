...
<script type="text/javascript" src="lib/jqui/jquery-ui-1.10.4.min.js"></script>
		...
		if (sid.length == 1) {
			if (confirm('Are you sure to delete section (ID: '+sid+')?')) {
				$.post( "<?php echo $this->url('task=stemplate&act=del-section&tid=' . $tid);?>", {sid:sid})
					.done(function( resp ) {
						//location.reload();
						window.location.href = "<?php echo $this->url('task=stemplate&act=section-tabdata&tid=' . $tid.'&sid='.$sid.'&tabid='.$tabid);?>";
					});
			}
		}
	});
...
.sec-head {
    border-bottom: 2px solid #9ac9de;
    font-size: 14px;
    padding-bottom: 3px;
}
.sec-api {
...
.edit-url .btn_add_section:hover{
	background: #ffffff;
...
<table width="100%">
<tr><td align="right"><a href="<?php echo $this->url('task=stemplate&act=new-section&tid='.$tid.'&tabid='.$tabid.'&tbsid='.$sid);?>" class="btn btn-mini btn-info btn_add_section">Add Section</a> &nbsp;&nbsp; <a href="#" onclick="save_template();" class="btn btn-mini btn-info">Save Template</a>
</td></tr>
...
?>
<div id="<?php echo $sec->section_id;?>" class="section"><div class="sec-del" alt="X"></div><div class="sec-head"><?php echo $sec->section_title;?></div>ID: <?php echo $sec->section_id;?>, Type: <?php echo $type;?>, API: <div class="sec-api"><?php echo htmlspecialchars($sec->api);?></div><div class="sec-type"><?php echo $sec->section_type;?></div><div class="sec-fields">
<?php 
...
?>
</div><?php if ($sec->section_type == 'F' || $sec->section_type == 'G') {?><div class="edit-url"><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=new-section&tid=' . $tid . '&sid='.$sec->section_id.'&tabid='.$tabid.'&tbsid='.$sid);?>">edit</a></div><?php } else if ($sec->section_type == 'D' || $sec->section_type == 'T' || $sec->section_type == 'I') {
?><div class="edit-url"><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=new-defined-section&tid=' . $tid . '&sid='.$sec->section_id.'&tabid='.$tabid.'&tbsid='.$sid);?>">edit</a></div><?php
} ?>
<div class="edit-url"><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=section-filters&tid=' . $tid . '&sid='.$sec->section_id.'&tabid='.$tabid.'&tbsid='.$sid);?>">filters</a></div>
<div class="edit-url"><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=section-values&tid=' . $tid . '&sid='.$sec->section_id.'&tabid='.$tabid.'&tbsid='.$sid);?>">values</a></div>
...
function save_template()
{
	...
	$.post( "<?php echo $this->url('task=stemplate&act=save-template&tid=' . $tid.'&tabid='.$tabid);?>", {data:jsonObj})
		.done(function( resp ) {
			location.reload();
	});
}
