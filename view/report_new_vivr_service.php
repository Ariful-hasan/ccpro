...
<div class="row">
    <div class="col-md-12 small">
        <form id="workcode-form" action="<?php echo $this->url("task=report-new&act=vivr-service-report"); ?>" class="form-inline" method="post" autocomplete="off">
            ...
            <a href="<?php echo $this->url("task=report-new&act=vivr-service-report&download=".md5("CC Workcode Report".session_id())) ?>" id="download-workcode-report" class=" btn btn-md btn-purple"><i class=" fa fa-download"></i> Download</a>
        </form>
        <div class="row">
            <br/>
        </div>
        ...
        <div class="table-responsive m-t-10">
            <table class="table table-bordered table-striped table-hover table-condensed service-level-table">
                ...
                if (empty($results)) {
                    echo "<tr>
                        <td class='text-danger' colspan='" . (count($date_list) + 2) . "'> No Record Found </td>
                        </tr>";
                } else {
                    foreach ($results as $result) {
                        echo "<tr>
                        <td class='text-success' > $result->service_name </td>" .
                        "<td > $result->main_menu_service </td>";
...
<script type="text/javascript">
    ...
    function prepareDownloadUrl(){
        ...
        url += '&sdate='+encodeURIComponent(sdate)+ '&edate='+ encodeURIComponent(edate) +
            '&ivr_id='+encodeURIComponent(ivr_id)+'&download=' + download;
