...
<script type="text/javascript">
	...
	<?php if (!empty($ivr_details)):?>
		...
		$('li a').contextMenu('myMenu1', {
			bindings: {
				'tn_edit': function(t) {
					requestNewWindow('<?php echo $this->url('task=ivrs&act=updatenode&ivrname=');?>'+ivr_old_name+'&menu='+ t.id.substring(8)+'&dtmfsign='+t.id.substring(5, 7), 530, 300);
				},
	
				'tn_add': function(t) {
					requestNewWindow('<?php echo $this->url('task=ivrs&act=addnode&ivrname=');?>'+ivr_old_name+'&menu='+ t.id.substring(8)+'&pevent='+t.id.substring(5, 7), 530, 300);
				},
				'tn_upload': function(t) {
					requestNewWindow('<?php echo $this->url('task=ivrs&act=upload-voice-file&ivrname=');?>'+ivr_old_name+'&branch='+ t.id.substring(8)+'&pevent='+t.id.substring(5, 7), 530, 300);
				},
				'tn_delete_only': function(t) {

					if(confirm('Are you sure to delete only this node?')) {
						$.getJSON("<?php echo $this->url('task=ivrs&act=deleteonlynode');?>", { menu: t.id.substring(8), ivrname:ivr_old_name },
							function(json){
...
//console.log($(e.target).parent('li').siblings('li').length);
				...
				if(event_name != 'PF' && event_name != 'AN' && event_name != 'UI' && event_name != 'SR' && event_name != 'MC' && event_name != 'CA' && event_name != 'RV' && event_name != 'SV' && event_name != 'SY' && event_name != 'GO' && event_name != 'IF' && event_name != 'SL' && event_name != 'AU') {
					$('#tn_add', menu).remove();
				}
...
#tn_edit, #tn_delete, #tn_add,#tn_upload,#tn_delete_only{
	font-size:11px;
}
</style>
		...
		<?php if (!empty($ivr_details)) {
			...
			foreach ($ivr_details as $_ivr) {
				...
				if ($_ivr->event == 'PF' || $_ivr->event == 'CA' || $_ivr->event == 'AU' || $_ivr->event == 'AN' || $_ivr->event == 'UI') {
					...
					if ($txt_event_val_length == 1 && $_ivr->event_key == substr($_ivr->branch, 0, 1)) {
