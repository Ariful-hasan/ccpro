...
var strSeatStatus = "hidden unreachable hold-out hold idle ring dial active activeo no-agent-talk chat-only chat-busy chat-call-busy enable";
var dimvalue = '<span class="dim-value">0</span>';
var wsUri = "<?php echo 'ws://'. $CCSettings->switch_ip . ':' . $ws_port;?>/chat";
...
$(document).ready(function(){
        ...
        $(".seat div.agent").hover(function(event) {
...
try {
        ...
        obj.loadSettings(wsUri,  '<?php echo UserAuth::getCurrentUser();?>', '<?php echo $token;?>', "<?php echo UserAuth::getDBSuffix();?>");
...
var _DashBoard = {
        ...
        PopulateSkill: function(skill_id, skill) {
                ...
                if (skill.skill_type == 'P' || skill.skill_type == 'A') {
                        if (skill.pd_in_progress != 0) {
                                $("#inPrg"+skill_id).html(skill.pd_in_progress);
                        } else {
                                $("#inPrg"+skill_id).html(dimvalue);
                        }
        ...
        PopulateSeat: function(seat_id, seat) {
                ...
                if (seat.agent_id.length != 4) {
                        ...
                        }else if (seat.status == 'H') {
                                $("#seat_"+seat_id).removeClass(strSeatStatus).addClass("hold");
                        }
                ...
                if (seat.status == 'I') {
                        ...
                        cssClass = "ring";
                } else if (seat.status == 'D' || seat.status == 'A') {
                        cssClass = "dial";
                ...
                $('#seat_'+seat_id+' div.status').html(agent_name +'<br>'+skill_name+'<br>'+status);
                $('#ts_'+seat_id).html('<span class="ts-fmt-lap">'+secondsToMMSS(lap)+'</span><span class="hidden ts-lap">'+getClientTime(lap)+'</span>');
...
function startTimer() {
        ...
        */
        ...
        $( "div.ts-data" ).each(function( index ) {
                sts = $( this ).find("span.ts-lap").text();
                sts = TimeDiff(curTs, sts);
                $( this ).find("span.ts-fmt-lap").text(secondsToMMSS(sts));
        });
...
#seats .seat.hold {
        background-image:url(image/agent_call_hold.gif);
}
    ...
    if (file_exists($image_src)) {
        ...
        $infoArray = array(
                ...
                array(
                        ...
                        array("img"=>"agent_call_hold.gif","desc"=>"Call in Hold"),
                ),
                 array(
...
<?php if ($skill->qtype == 'P'):?>

<tr><td colspan="2" class="qhead-td"><div class="qhead"><?php echo $skill->short_name . ' [PD]';?></div></td></tr>


<tr class="qinfo-tr"><td><div class="qinfo">In Queue</div></td><td class="qbox-col-left-border"><div class="qinfo">In Service</div></td></tr>
<tr>
...
<tr class="qinfo-tr"><td colspan="2"><div class="qinfo">In Progress</div></td></tr>
<tr><td colspan="2"><div id="inPrg<?php echo $skill->skill_id;?>" class='valbox'><span class="dim-value">0</span></div></td></tr>



<?php else:?>

<tr><td colspan="2" class="qhead-td"><div class="qhead"><?php echo $skill->short_name; if ($skill->qtype == 'P') echo ' [PD]'; else if ($skill->qtype == 'A') echo ' [AN]';?></div></td></tr>
<tr class="qinfo-tr"><td><div class="qinfo"><?php echo $skill->qtype == 'out' ? 'In Progress' : 'In Queue';?></div></td><td class="qbox-col-left-border"><div class="qinfo">In Service</div></td></tr>
<tr>
        <td style='position:relative;'><div id="inq<?php echo $skill->skill_id;?>" class='valbox'><span class="dim-value">0</span></div>
        <div id='inqLang<?php echo $skill->skill_id;?>' class="q-lang language">
...
<tr class="qinfo-tr"><td><div class="qinfo"><?php if ($skill->qtype == 'P') echo 'In Progress'; elseif ($skill->qtype == 'A') echo 'Call Count'; else echo 'AWT';?></div></td><td class="qbox-col-left-border"><div class="qinfo"><?php echo $skill->type == 'out' ? 'MTT' : 'MWT';?></div></td></tr>
<?php if ($skill->qtype == 'P' || $skill->qtype == 'A'):?>
<tr><td><div id="inPrg<?php echo $skill->skill_id;?>" class='valbox'><span class="dim-value">0</span></div></td><td class="qbox-col-left-border"><div id="mt<?php echo $skill->skill_id;?>" class='valbox'><span class="dim-value">0</span></div></td></tr>
<?php else:?>
...
<div id="seat_<?php echo $seat->seat_id;?>" class="seat empty hidden" style='position:relative;'>
        <div class="agentpic" id="ap_<?php echo $seat->seat_id;?>" agent=""><?php echo $seat->label;?></div>
        <div class="seat-text">
        <div class="agent"></div>
        <div class="status"></div>
        <div class="seatinfo">&nbsp;</div>
    </div>
    <div id="ts_<?php echo $seat->seat_id;?>" class="ts-data">&nbsp;</div>
...
<script>
        ...
        $("#btnInfo").mouseover(function(){
                ...
                $("#infoContainer").css({"left":left+width-600+'px',"top":top+height+10}).show();
                //$("#infoContainer").show();
        }).mouseout(function(){
                $("#infoContainer").hide();
        });
