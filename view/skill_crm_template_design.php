...
$(document).ready(function() {
	...
	*/
	...
	$(".sec-del").click(function (e) {
		...
		//$.colorbox({inline:true, onClosed:function(){ $('#add-sec-elm').hide();}, onOpen:function(){ $('#add-sec-elm').show();}, height:500, width:620, href:"#add-sec-elm"});
		if (sid.length == 1) {
			if (confirm('Are you sure to delete section (ID: '+sid+')?')) {
				$.post( "<?php echo $this->url('task=stemplate&act=del-section&tid=' . $tid);?>", {sid:sid})
					.done(function( resp ) {
						//location.reload();
						window.location.href = "<?php echo $this->url('task=stemplate&act=details&tid=' . $tid);?>";
					});
			}
		}
	});
	
	$("a.btn_add_section").colorbox({iframe:true, height:500, width:820});
	$("a.btn_tab_section").colorbox({iframe:true, height:'95%', width:'95%'});
});
...
<?php 
	...
	if (is_array($fields)) {
		foreach ($fields as $fld) {
			$secid = empty($fld->section_id) ? 'SR' : $fld->section_id;
			$flds[$secid][] = $fld;
		}
	}
...
<table width="100%">
<tr><td align="right"><a href="<?php echo $this->url('task=stemplate&act=new-defined-section&tid=' . $tid);?>" class="btn btn-mini btn-info btn_add_section">Add Pre-Defined Section</a> &nbsp;&nbsp; <a href="<?php echo $this->url('task=stemplate&act=new-section&tid=' . $tid);?>" class="btn btn-mini btn-info btn_add_section">Add Section</a> &nbsp;&nbsp; <a href="#" onclick="save_template();" class="btn btn-mini btn-info">Save Template</a>
</td></tr>
<tr><td width="100%" valign="top" id="td-sections">

<div id="SR" class="section"><div class="sec-head">CRM Search</div>ID: -, Type: Search, API: <div class="sec-api"><?php echo htmlspecialchars($template_details->api);?></div>
<div><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=section-filters&tid=' . $tid . '&sid=');?>">filters</a></div>
<div><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=section-values&tid=' . $tid . '&sid=');?>">values</a></div>
...
?>
<div id="<?php echo $sec->section_id;?>" class="section"><div class="sec-del btn btn-danger btn-xs" alt="X"><i class="fa fa-times"></i></div><div class="sec-head"><?php echo $sec->section_title;?></div>ID: <?php echo $sec->section_id;?>, Type: <?php echo $type;?>, API: <div class="sec-api"><?php echo htmlspecialchars($sec->api);?></div><div class="sec-type"><?php echo $sec->section_type;?></div><div class="sec-fields">
<?php

if (isset($flds[$sec->section_id])) {
	foreach ($flds[$sec->section_id] as $fld) {
	    $tabUrlId = $fld->field_tab_id;
?>
<div class="sec-field">
    <div class="sec-fld-label"><?php echo $fld->field_label;?></div>
    <div class="sec-fld-val">
    <?php 
    if (!empty($fld->field_tab_id)){
        echo '<a class="btn_tab_section" href="'.$this->url('task=stemplate&act=section-tabdata&tid=' . $tid . '&sid='.$sec->section_id.'&tabid='.$fld->field_tab_id).'">'.$fld->field_key.'</a>';
    }else { 
        echo $fld->field_key;
    }
    ?>
...
?>
</div><?php if ($sec->section_type == 'F' || $sec->section_type == 'G') {?><div><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=new-section&tid=' . $tid . '&sid='.$sec->section_id);?>">edit</a></div><?php } else if ($sec->section_type == 'D' || $sec->section_type == 'T' || $sec->section_type == 'I') {
?><div><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=new-defined-section&tid=' . $tid . '&sid='.$sec->section_id);?>">edit</a></div><?php
} ?>
<div><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=section-filters&tid=' . $tid . '&sid='.$sec->section_id);?>">filters</a></div>
<div><a class="btn_add_section" href="<?php echo $this->url('task=stemplate&act=section-values&tid=' . $tid . '&sid='.$sec->section_id);?>">values</a></div>
<div class="sec-clear"></div></div>
<?php
	}
}
...
function save_template()
{
	...
	$.post( "<?php echo $this->url('task=stemplate&act=save-template&tid=' . $tid);?>", {data:jsonObj})
		.done(function( resp ) {
			location.reload();
	});
	//console.log(jsonObj);
...
function load_section(sec_id, stype)
{
	...
	if (sec_id.length > 0) {
		...
		if (sec.length) {
			...
			curSection = sec_id;
			//alert(sectitle);
			$('#section_title').val(sectitle);
			//console.log($('#section_title').val());
	...
	if (!is_div_exist) {
		...
		for(var i = first.charCodeAt(0); i <= last.charCodeAt(0); i++) {
			...
			ch = String.fromCharCode(i);
			//alert($('#'+ch).length);
			if ($('#'+ch).length == 0) {$('#section_id').val(ch); break;}
		}
	}
	
	
	$.colorbox({inline:true, onClosed:function(){ $('#add-sec-elm').hide();}, onOpen:function(){ $('#add-sec-elm').show();}, height:500, width:720, href:"#add-sec-elm"});
}
