...
if (isset($_REQUEST['download'])) {
    ...
    $dl_helper->write_in_file(",Duration(sec)>>,<5,5-10,11-20,21-30,31-45,46-59,>=60,<5,5-10,11-20,21-30,31-45,46-59,>=60\n");
    if (is_array($logs)) {
        $i = 0;
        $total_answered_l5 = 0;
...
<?php if (!empty($errMsg)):?><?php if ($errType === 0):?><div class="alert alert-success"><?php else:?><div class="alert alert-error"><?php endif;?> <?php echo $errMsg;?></div><?php endif;?>
        ...
        <table class="form_table">
            <tr class="form_row">
                <td>
                    ...
                    From:
                    <input type="text" name="sdate" id="sdate" value="<?php echo $from;?>" class="form-control">
                    <!--<input type="text" name="stime" id="stime" value="<?php /*echo $dateinfo->stime;*/?>" size="5" maxlength="5">--> &nbsp;&nbsp;
                    To:
                    <input type="text" name="edate" id="edate" value="<?php echo $to;?>" class="form-control">
    ...
    <table class="report_table" width="98%" border="0" align="center" cellpadding="1" cellspacing="1">
        ...
        <tr class="report_row_head" height=20 align="center">
            ...
            <td class="cntr">11-20</td>
            <td class="cntr">21-30</td>
            <td class="cntr">31-45</td>
            <td class="cntr">46-59</td>
            <td class="cntr">&gt;=60</td>
            <td class="cntr">&lt;5</td>
            <td class="cntr">5-10</td>
            <td class="cntr">11-20</td>
        ...
        <tr>
            <td>
                <?php if (is_array($logs)):?>
                    ...
                    $dl_link = "download=$download&skill=$skill&sdate=$from&edate=$to";
                    $url = $this->url('task='.$request->getControllerName().'&act='.$request->getActionName().'&'.$dl_link);
                    ?>
                    <img class="bottom" height="16" width="20" border="0" src="image/down_excel.gif"/><a id="skill_spectrum_download" class="btn-link" href="<?php echo $url;?>">download current records</a>
                <?php endif;?>
                &nbsp;
            </td>
        </tr>
    ...
    <script>
        $(document).on("click","#skill_spectrum_download",function () {
            $("#MainLoader").remove();
        });
