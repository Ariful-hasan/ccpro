...
if (isset($_REQUEST['download'])) {
	...
	$t = time();
	$dl_helper->create_file($t . '_cdr_download_outbound.csv');
	$dl_helper->write_in_file("SL,Start time,Stop time,Call from,Call to,Duration\r\n");
	$i = 0;
	$num_rows = 500;
	
	while ($num_rows == 500) {
        $logs = $report_model->getDLoadOutboundCDR($dateinfo, $i, 500);
        $num_rows = 0;
        if (is_array($logs)) {
            $num_rows = count($logs);
    		foreach ($logs as $log) {
        		$i++;
        		$duration = gmdate("H:i:s", $log->talk_time);
        		//$talktime = empty($log->service_time) ? '00:00:00' : gmdate("H:i:s", $log->service_time);
        		$stop_time = date('Y-m-d H:i:s', strtotime('+'.$log->service_time.'seconds', strtotime($log->start_time)));
    			$dl_helper->write_in_file("$i,$log->start_time,$stop_time,$log->callerid,$log->callto,$duration\r\n");
        	}
        }
	}
...
<form name="frm_search" method="post" action="<?php echo $this->url('task='.$request->getControllerName()."&act=".$request->getActionName()."&type=outbound");?>">
...
<tr>
	...
	<?php if ($pagination->num_records <= 40000) {
	        ...
	        $session_id = session_id();
                ...
                $dl_link = "type=outbound&download=$download&skill=$skill&sdate=$dateinfo->sdate&edate=$dateinfo->edate&stime=$dateinfo->stime&etime=$dateinfo->etime";
...
<tr class="<?php echo $_class;?>">
	...
	<td class="cntr">&nbsp;<?php echo $log->callerid;?></td>
	<td class="cntr">&nbsp;<?php echo $log->callto;?></td>
	<td class="cntr">&nbsp;<?php echo gmdate("H:i:s", $log->talk_time);?></td>
</tr>
<?php endforeach;?>
</table>
